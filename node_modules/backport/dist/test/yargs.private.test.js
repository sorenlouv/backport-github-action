...
function getGlobalConfig() {
    return JSON.parse(child_process_1.execSync(`./node_modules/.bin/ts-node --transpile-only ./src/test/getGlobalConfig.ts`, execOptions));
}
describe('yargs', () => {
    let accessToken;
    let username;
    ...
    it('should return error when branch is missing', () => {
        const res = runBackport(`--upstream foo --username ${username} --accessToken ${accessToken}`);
    ...
    it('should return error when upstream is missing', () => {
        const res = runBackport(`--branch foo --username ${username} --accessToken ${accessToken}`);
        expect(res).toMatchInlineSnapshot(`
    ...
    it('should return error when access token is invalid', () => {
        const res = runBackport(`--branch foo --upstream foo/bar  --username some-user --accessToken some-token`);
        expect(res).toContain('Please check your access token and make sure it is valid');
    });
    it(`should return error when repo doesn't exist`, () => {
        const res = runBackport(`--branch foo --upstream foo/bar --username ${username} --accessToken ${accessToken}`);
        expect(res).toMatchInlineSnapshot(`
...
function runBackport(args) {
    const cmd = `./node_modules/.bin/ts-node --transpile-only ./src/entrypoint.cli.ts ${args}`;
    return child_process_1.execSync(cmd, execOptions);
}
function runBackportAsync(options) {
    const proc = child_process_1.spawn('./node_modules/.bin/ts-node', [
        '--transpile-only',
    ...
    return new Promise((resolve) => {
        ...
        proc.stdout.on('data', (dataChunk) => {
            ...
            if (output.includes('Select commit')) {
                ...
                resolve(strip_ansi_1.default(output).replace(/\s+$/gm, ''));
                proc.kill();
            }
        });
    });
