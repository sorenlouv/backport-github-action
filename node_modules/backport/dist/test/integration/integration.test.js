...
const createSpies_1 = require("./createSpies");
...
describe('integration', () => {
    ...
    describe('when a single commit is backported', () => {
        let res;
        let spies;
        beforeEach(lodash_1.once(async () => {
            jest.clearAllMocks();
            spies = createSpies_1.createSpies({ commitCount: 1 });
            await helpers_1.deleteAndSetupEnvironment();
        ...
        it('should not create new branches in origin (backport-org/backport-demo)', async () => {
            const branches = await helpers_1.getBranches(envConstants_1.REMOTE_ORIGIN_REPO_PATH);
            expect(branches).toEqual(['6.0', '* master']);
        });
        it('should create branch in the fork (sqren/backport-demo)', async () => {
            const branches = await helpers_1.getBranches(envConstants_1.REMOTE_FORK_REPO_PATH);
            expect(branches).toEqual(['6.0', 'backport/6.0/pr-85', '* master']);
        });
        it('should have cherry picked the correct commit', async () => {
            const commit = await helpers_1.getLatestCommit({
                branch: 'backport/6.0/pr-85',
                commitCount: 1,
                cwd: envConstants_1.REMOTE_FORK_REPO_PATH,
            });
    ...
    describe('when two commits are backported', () => {
        ...
        beforeEach(lodash_1.once(async () => {
            jest.clearAllMocks();
            spies = createSpies_1.createSpies({ commitCount: 2 });
            await helpers_1.deleteAndSetupEnvironment();
            const options = await options_1.getOptions([], {
                // use localhost to avoid CORS issues
                githubApiBaseUrlV4: 'http://localhost/graphql',
    ...
    describe('when disabling fork mode', () => {
        ...
        beforeEach(lodash_1.once(async () => {
            ...
            spies = createSpies_1.createSpies({ commitCount: 1 });
            await helpers_1.deleteAndSetupEnvironment();
            const options = await options_1.getOptions(['--fork=false'], {
                // use localhost to avoid CORS issues
                githubApiBaseUrlV4: 'http://localhost/graphql',
            });
            res = await runWithOptions_1.runWithOptions(options);
        ...
        it('should cherry pick the correct commit', async () => {
            const commit = await helpers_1.getLatestCommit({
                branch: 'backport/6.0/pr-85',
                commitCount: 1,
                cwd: envConstants_1.REMOTE_ORIGIN_REPO_PATH,
            });
            expect(commit).toMatchInlineSnapshot(`
      " romeo-and-juliet.txt | 2 +-
       1 file changed, 1 insertion(+), 1 deletion(-)
