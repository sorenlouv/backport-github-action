...
describe('getUnstagedFiles', () => {
    it('should split lines and remove empty', async () => {
        ...
        await expect(await git_1.getUnstagedFiles(options)).toEqual([
...
describe('getConflictingFiles', () => {
    it('should split by linebreak and remove empty and duplicate items', async () => {
        ...
        expect(await git_1.getConflictingFiles(options)).toEqual([
            '/myHomeDir/.backport/repositories/elastic/kibana/conflicting-file.txt',
        ]);
    });
});
describe('createFeatureBranch', () => {
    ...
    it(`should handle "couldn't find remote ref" error`, async () => {
        ...
        jest.spyOn(childProcess, 'exec').mockRejectedValueOnce(err);
        await expect(git_1.createBackportBranch({ options, targetBranch, backportBranch })).rejects.toThrowErrorMatchingInlineSnapshot(`"The branch \\"4.x\\" is invalid or doesn't exist"`);
    });
    ...
    it('should rethrow normal error', async () => {
        ...
        jest.spyOn(childProcess, 'exec').mockRejectedValueOnce(err);
        expect.assertions(1);
        await expect(git_1.createBackportBranch({ options, targetBranch, backportBranch })).rejects.toThrowErrorMatchingInlineSnapshot(`"just a normal error"`);
    });
});
describe('deleteRemote', () => {
    ...
    it('should swallow exec error', async () => {
        ...
        jest.spyOn(childProcess, 'exec').mockRejectedValueOnce(err);
        await expect(await git_1.deleteRemote(options, remoteName)).toBe(undefined);
    });
    it('should rethrow normal error', async () => {
        const err = new Error('just a normal error');
        jest.spyOn(childProcess, 'exec').mockRejectedValueOnce(err);
        expect.assertions(1);
        await expect(git_1.deleteRemote(options, remoteName)).rejects.toThrowErrorMatchingInlineSnapshot(`"just a normal error"`);
    });
});
describe('cherrypick', () => {
    const options = {
    ...
    it('should return `needsResolving: false` when no errors are encountered', async () => {
        jest
            ...
            .mockResolvedValueOnce({ stderr: '', stdout: '' });
        expect(await git_1.cherrypick(options, commit)).toEqual({
    ...
    it('should use mainline option when specified', async () => {
        const execSpy = jest
            ...
            // mock cherry pick command
            .mockResolvedValueOnce({ stderr: '', stdout: '' });
        await git_1.cherrypick({ ...options, mainline: 1 }, commit);
    ...
    it('should return `needsResolving: true` upon cherrypick error', async () => {
        jest
            ...
            .mockResolvedValueOnce({ stderr: '', stdout: '' })
            // mock cherry pick command
            .mockRejectedValueOnce(new ExecError_1.ExecError({
            killed: false,
            code: 128,
    ...
    it('it should let the user know about the "--mainline" argument when cherry-picking a merge commit without specifying it', async () => {
        ...
        await expect(git_1.cherrypick(options, commit)).rejects
...
Or refer to the git documentation for more information: https://git-scm.com/docs/git-cherry-pick#Documentation/git-cherry-pick.txt---mainlineparent-number`);
    ...
    it('it should gracefully handle empty commits', async () => {
        ...
        await expect(git_1.cherrypick(options, commit)).rejects.toThrowError(`Cherrypick failed because the selected commit (abcd) is empty. This is most likely caused by attemping to backporting a commit that was already backported`);
    });
    it('should re-throw non-cherrypick errors', async () => {
        jest
            ...
            // getUnstagedFiles
            .mockResolvedValueOnce({ stdout: '', stderr: '' });
        await expect(git_1.cherrypick(options, commit)).rejects.toThrowErrorMatchingInlineSnapshot(`"non-cherrypick error"`);
    });
});
describe('cherrypickContinue', () => {
    ...
    it('should swallow error if changes have already been committed manaully', async () => {
        ...
        jest.spyOn(childProcess, 'exec').mockRejectedValueOnce(err);
        await expect(await git_1.finalizeCherrypick(options)).toBe(undefined);
    });
    it('should re-throw other errors', async () => {
        const err = new Error('non-cherrypick error');
        jest.spyOn(childProcess, 'exec').mockRejectedValueOnce(err);
        expect.assertions(1);
        await expect(git_1.finalizeCherrypick(options)).rejects.toThrowErrorMatchingInlineSnapshot(`"non-cherrypick error"`);
    });
});
describe('addRemote', () => {
    const options = {
    ...
    it('add correct origin remote', async () => {
        const spy = jest
            .spyOn(childProcess, 'exec')
            .mockResolvedValueOnce({ stderr: '', stdout: '' });
        await git_1.addRemote(options, 'elastic');
        return expect(spy).toHaveBeenCalledWith('git remote add elastic https://myAccessToken@github.com/elastic/kibana.git', { cwd: '/myHomeDir/.backport/repositories/elastic/kibana' });
    });
    it('add correct user remote', async () => {
        const spy = jest
            .spyOn(childProcess, 'exec')
            .mockResolvedValueOnce({ stderr: '', stdout: '' });
        await git_1.addRemote(options, 'sqren');
        return expect(spy).toHaveBeenCalledWith('git remote add sqren https://myAccessToken@github.com/sqren/kibana.git', { cwd: '/myHomeDir/.backport/repositories/elastic/kibana' });
    });
    it('allows custom github url', async () => {
        const spy = jest
            .spyOn(childProcess, 'exec')
            .mockResolvedValueOnce({ stderr: '', stdout: '' });
        await git_1.addRemote({ ...options, gitHostname: 'github.my-company.com' }, 'sqren');
        return expect(spy).toHaveBeenCalledWith('git remote add sqren https://myAccessToken@github.my-company.com/sqren/kibana.git', { cwd: '/myHomeDir/.backport/repositories/elastic/kibana' });
    });
});
describe('pushBackportBranch', () => {
    ...
    it('should handle missing fork error', async () => {
        ...
        await expect(git_1.pushBackportBranch({ options, backportBranch })).rejects.toThrowErrorMatchingInlineSnapshot(`"Error pushing to https://github.com/sqren/kibana. Repository does not exist. Either fork the source repository (https://github.com/elastic/kibana) or disable fork mode \\"--fork false\\".  Read more about \\"fork mode\\" in the docs: https://github.com/sqren/backport/blob/3a182b17e0e7237c12915895aea9d71f49eb2886/docs/configuration.md#fork"`);
    });
});
