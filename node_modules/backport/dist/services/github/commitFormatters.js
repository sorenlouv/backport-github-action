"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.getFormattedCommitMessage = exports.getFirstCommitMessageLine = exports.getShortSha = void 0;
function getShortSha(sha) {
    return sha.slice(0, 8);
}
exports.getShortSha = getShortSha;
function getFirstCommitMessageLine(message) {
    return message.split('\n')[0];
}
exports.getFirstCommitMessageLine = getFirstCommitMessageLine;
function getFormattedCommitMessage({ message, pullNumber, sha, }) {
    const firstMessageLine = getFirstCommitMessageLine(message);
    const messageHasPullNumber = firstMessageLine.match(/.+ \(#\d+\)/);
    // message already contains pull number
    if (messageHasPullNumber) {
        return firstMessageLine;
    }
    // message doesn't contain pull number. Add it
    if (pullNumber) {
        return `${firstMessageLine} (#${pullNumber})`;
    }
    // pull number not available. Add commit
    return `${firstMessageLine} (${getShortSha(sha)})`;
}
exports.getFormattedCommitMessage = getFormattedCommitMessage;
